...
public class SchedulesRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    private void updateRendering ( int width, int height )
    {
        ...
        transition3Dto2D = new Transition3Dto2D ( mScreenWidth, mScreenHeight, isActivityInPortraitMode, mDPIScaleIndicator, mScaleFactor, mPlane );
        transition3Dto2D.initializeGL ( shaderProgramID );

        // Initialize the 2D to 3D Transition
        transition2Dto3D = new Transition3Dto2D ( mScreenWidth, mScreenHeight, isActivityInPortraitMode, mDPIScaleIndicator, mScaleFactor, mPlane );
    ...
    public void renderFrame ( State state, float[] projectionMatrix )
    {
        ...
        if ( state.getNumTrackableResults ( ) > 0 )
        {
            ...
            String status = stringForStatus ( trackableResult.getStatus ( ) );
            String statusInfo = stringForStatusInfo ( trackableResult.getStatusInfo ( ) );
            Log.d ( "SchedulesRenderer", "Status : " + status + " | Status Info : " + statusInfo );
        ...
        else
        {
            ...
            if ( !mScanningMode && mShowAnimation3Dto2D && renderState == RS_NORMAL && framesToSkipBeforeRenderingTransition.get ( ) == 0 )
            {
                startTransitionTo2D ( );
            }
        ...
        if ( mProductTexture == null )
        {
            ...
            mActivity.setStatusBarText ( "Searching" );
    ...
    private void renderAugmentation ( TrackableResult trackableResult, float[] projectionMatrix )
    {
        ...
        Matrix.scaleM ( modelViewMatrix, 0, 0.430f * mScaleFactor, 0.430f * mScaleFactor, 1.0f );
    ...
    private void generateProductTextureInOpenGL ( )
    {
        ...
        GLES20.glTexSubImage2D ( GLES20.GL_TEXTURE_2D, 0, 0, 0, mProductTexture.mWidth, mProductTexture.mHeight, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, mProductTexture.mData );
